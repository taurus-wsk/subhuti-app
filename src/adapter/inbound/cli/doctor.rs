...
fn print_doctor_output(results: &[CheckResult]) {
    ...
    if failed == 0 {
        ...
        println!("   subhuti api orchestrate --message '你好' # 测试 API");
...
fn source_code_check(results: &mut Vec<CheckResult>) {
    let src_exists = std::path::Path::new("crates/subhuti/src/lib.rs").exists();
    let docs_exist = std::path::Path::new("docs").exists();
