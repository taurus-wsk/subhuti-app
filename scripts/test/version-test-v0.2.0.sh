...
test_new_features() {
    echo ""
    echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}新功能测试 - v0.2.0 (日志查询增强)${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
    
    # 测试 1: 日志级别过滤
    echo "🔍 日志级别过滤..."
    LOGS=$(curl -sf "$BASE_URL/logs?level=ERROR&limit=5" 2>&1)
    if echo "$LOGS" | grep -q '"logs"'; then
        LEVEL_CHECK=$(echo "$LOGS" | python3 -c "import sys,json; logs=json.load(sys.stdin).get('data',{}).get('logs',[]); print(all(l.get('level')=='ERROR' for l in logs))" 2>/dev/null)
        if [ "$LEVEL_CHECK" = "True" ]; then
            record_test "日志级别过滤" "PASS" "所有日志都是 ERROR 级别"
        else
            record_test "日志级别过滤" "FAIL" "级别过滤不正确"
        fi
    else
        record_test "日志级别过滤" "FAIL" "API 返回异常"
    fi
    
    # 测试 2: 时间范围查询
    echo "⏰ 时间范围查询..."
    LOGS=$(curl -sf "$BASE_URL/logs?limit=5" 2>&1)
    if echo "$LOGS" | grep -q '"logs"'; then
        # 获取第一条日志的时间
        FIRST_TIME=$(echo "$LOGS" | python3 -c "import sys,json; logs=json.load(sys.stdin).get('data',{}).get('logs',[]); print(logs[0]['timestamp'] if logs else '')" 2>/dev/null)
        if [ -n "$FIRST_TIME" ]; then
            record_test "时间范围查询" "PASS" "API 正常，示例时间: $FIRST_TIME"
        else
            record_test "时间范围查询" "WARN" "无日志数据"
        fi
    else
        record_test "时间范围查询" "FAIL" "API 返回异常"
    fi
    
    # 测试 3: 分页功能
    echo "📄 分页功能..."
    LOGS1=$(curl -sf "$BASE_URL/logs?limit=2&offset=0" 2>&1)
    LOGS2=$(curl -sf "$BASE_URL/logs?limit=2&offset=2" 2>&1)
    if echo "$LOGS1" | grep -q '"logs"' && echo "$LOGS2" | grep -q '"logs"'; then
        COUNT1=$(echo "$LOGS1" | python3 -c "import sys,json; print(len(json.load(sys.stdin).get('data',{}).get('logs',[])))" 2>/dev/null)
        COUNT2=$(echo "$LOGS2" | python3 -c "import sys,json; print(len(json.load(sys.stdin).get('data',{}).get('logs',[])))" 2>/dev/null)
        if [ "$COUNT1" -le 2 ] && [ "$COUNT2" -le 2 ] 2>/dev/null; then
            record_test "分页功能" "PASS" "第1页: $COUNT1 条, 第2页: $COUNT2 条"
        else
            record_test "分页功能" "FAIL" "分页异常"
        fi
    else
        record_test "分页功能" "FAIL" "API 返回异常"
    fi
}
